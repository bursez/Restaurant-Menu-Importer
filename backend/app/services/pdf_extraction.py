...
def clean_repeated_pdf_chrome(pages: list[PdfPageText]) -> tuple[list[PdfPageText], list[str]]:
    ...
    repeated_keys = {
        ...
        if count >= threshold
        and (len(original_by_normalized[key]) <= 100 or LEGAL_OR_LEGEND_PATTERN.search(original_by_normalized[key]))
    }
    ...
    for page, lines in zip(pages, page_lines, strict=True):
        ...
        cleaned_pages.append(
            PdfPageText(page_number=page.page_number, text=text, line_count=text.count("\n") + 1 if text else 0)
        )
...
def extract_pdf_text_with_ocr(pdf_content: bytes) -> PdfExtractionResult:
    try:
        ...
        return PdfExtractionResult(
            ...
            pages=result.pages,
            method="ocrmypdf",
            warnings=["PDF text layer was empty; OCR fallback used", *result.warnings],
        )
